...
- **Cadastro de empresa + marca no relatório (feito)** — `login.html` tem seletor "Sou cliente / Sou empresa". Empresa informa **CNPJ**, vira `role: empresa_admin`, cria seu doc `empresas/{id}` (ownerUid = ele) e já entra vinculado. Empresa doc ganhou: `cnpj, corRelatorio, emailRelatorio, telefoneRelatorio`.
  `configuracoes.html`: card "Identidade no Relatório" (só empresa_admin/master) edita nome/e-mail/telefone/**cor** com prévia; salva na empresa e atualiza o cache.
  `oliver_pdf.js`: cabeçalho usa a **marca da empresa** (nome + "email — telefone" + cor, com contraste automático). Marca vem de `window.oliverBranding`/`localStorage` (populado por `app.html` ao logar e por configurações ao salvar). Rodapé novo: "Relatório gerado no OliverApp - ... https://olivewar.vercel.app/". Regras: empresa cria/edita a própria empresa; usuário pode ser criado com role empresa_admin.
  PENDENTE p/ empresa_admin ser 100% usável: poder **vincular clientes** (hoje só master faz em `empresas.html`; falta regra que deixe empresa_admin setar `empresaId` de outro usuário para a própria empresa) — fica na Fase 3.
